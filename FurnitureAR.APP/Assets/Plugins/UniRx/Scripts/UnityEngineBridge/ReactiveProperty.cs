...
namespace UniRx
{
    ...
    /// <summary>
    /// САМОПИЛ: Разработчики что ваще ебланы почему нет без инициализации ссука
    /// </summary>
    [Serializable]
    public class ReactivePropertyNoInit<T> : IReactiveProperty<T>, IDisposable, IOptimizedObservable<T>, IObserverLinkedList<T>
    {
#if !UniRxLibrary
        static readonly IEqualityComparer<T> defaultEqualityComparer = UnityEqualityComparer.GetDefault<T>();
#else
